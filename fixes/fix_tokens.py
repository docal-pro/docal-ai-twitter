...
def get_historical_price(token_id: str, date: datetime, max_retries=3) -> float:
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"Attempt {attempt + 1}/{max_retries} for historical price of {token_id} on {date_str}"
            )
...
def get_current_price(token_id: str, max_retries=3) -> float:
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"Attempt {attempt + 1}/{max_retries} for current price of {token_id}"
            )
            response = requests.get(url, headers=headers, params=params, timeout=30)
...
# Read the CSV file
df = pd.read_csv(os.path.expanduser("./results/evaluated_results_frank.csv"))
...
for tweet_id, fix_info in specific_fixes.items():
    ...
    if any(mask):
        ...
        if starting_price is not None and ending_price is not None:
            ...
            print(
                f"Updated prices - Start: {starting_price:.6f}, End: {ending_price:.6f}"
            )
        else:
...
df.to_csv(os.path.expanduser("./results/evaluated_results_frank.csv"), index=False)
print("\nUpdates complete!")
